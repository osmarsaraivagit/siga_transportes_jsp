...
//definir qual a pasta a ser salva
    ...
    if (up.formProcess(getServletContext(), request)) {
        ...
        try {
            ...
            if(id.equals("")){
                st.executeUpdate("INSERT into tesoureiros (nome, cpf, telefone, email, endereco, foto) values ('" + nome + "', '" + cpf + "' , '" + telefone + "' , '" + email + "', '" + endereco + "', '" + imagem + "')");
                st.executeUpdate("INSERT into usuarios (nome, cpf, email, senha, nivel, foto) values ('" + nome + "', '" + cpf + "', '" + email + "', '123', 'tesoureiro', '" + imagem + "')");
            }else{
                if(imagem.equals("sem-foto.jpg")){
                    st.executeUpdate("UPDATE tesoureiros SET nome = '" + nome + "', cpf = '" + cpf + "', telefone = '" + telefone + "', email = '" + email + "', endereco = '" + endereco + "' WHERE id = '" + id + "'");
                    st.executeUpdate("UPDATE usuarios SET nome = '" + nome + "', cpf = '" + cpf + "', email = '" + email + "' WHERE cpf = '" + antigo + "'");
                }else{
                    st.executeUpdate("UPDATE tesoureiros SET nome = '" + nome + "', cpf = '" + cpf + "', telefone = '" + telefone + "', email = '" + email + "', endereco = '" + endereco + "', foto = '" + imagem + "' WHERE id = '" + id + "'");
